"""
ALOHA + Joint grants and resource allocation
"""
...
def main():
    Bernard = SummaryWriter(comment='_BM1')    # Assigning an instance of SummaryWriter class to Bernard variable, to be used to write summaries to TensorBoard log files.
    ...
    for loops in range(args.loops):   # Running the loop for given number of iterations
        ...
        while not environment.done:   # While the environment has not been completed (i.e., done=False)
            ...
            requests = torch.rand(d_output_ue*n_client).round().to(device)
            # requests = torch.ones(d_output_ue*n_client)
