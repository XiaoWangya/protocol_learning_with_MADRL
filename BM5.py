    ...
    for loops in range(args.loops):   # Running the loop for given number of iterations
        ...
        while not environment.done:   # While the environment has not been completed (i.e., done=False)
            ...
            for i in range(n_client):
                if environment.statbility_info['indicator'][i] < args.threshold* environment.stability_constant[i]:
                    requests[i] *= 0
        ...
        Bernard.add_scalar('Efficient energy consumption in requesting', sum(eff_request_eng)/sum(request_eng), global_step=loops)
        Bernard.add_scalar('Energy consumption in transmission', sum(transmit_eng), global_step=loops)
        Bernard.add_scalar('Efficient energy consumption in transmission', sum(eff_transmit_eng)/sum(transmit_eng), global_step=loops)
