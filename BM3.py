...
def main():
    Bernard = SummaryWriter(comment='_BM3')    # Assigning an instance of SummaryWriter class to Bernard variable, to be used to write summaries to TensorBoard log files.
    ...
    for loops in range(args.loops):   # Running the loop for given number of iterations
        ...
        while not environment.done:   # While the environment has not been completed (i.e., done=False)
            ...
            action_bs_r = torch.cat((torch.rand(n_client).to(device).unsqueeze(0), action_bs_r), 0)  # Getting predicted action for BS from actor network, which takes current BS state as input
